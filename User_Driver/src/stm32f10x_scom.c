...
/** @defgroup SCOM_Private_TypesDefinitions
  * @{
  */
...
/** 
  * @brief SCOM_RX_Buffer structure definition
  */
		...
		{			
				...
				else       
				{
					  ...
					  cnt[0] = 0;
						...
						if(SCOMx->SCOM_Operating.txCallbackFun != NULL)
								(*SCOMx->SCOM_Operating.txCallbackFun)();							
				}
	  }
    else if(SCOMx == SCOM2)
		{			
...
void SCOM_StructInit(SCOM_InitTypeDef_Struct* SCOMx_InitStruct)
{
    ...
    SCOMx_InitStruct->SCOM_USART_SubPriority             = SCOM_USART_SUBPRIORITY_DEFAULT;
	  ...
	  SCOMx_InitStruct->SCOM_DMA_SubPriority               = SCOM_DMA_SUBPRIORITY_DEFAULT;
		...
		SCOMx_InitStruct->rxCallbackFun  = NULL;
		SCOMx_InitStruct->txCallbackFun  = NULL;		
}
...
ErrorStatus SCOM_Init(SCOM_TypeDef_Struct* SCOMx, SCOM_InitTypeDef_Struct* SCOMx_InitStruct)
{
    ...
    if(SCOMx->SCOM_Operating.enable_DMA != DISABLE)	
    {
		...
		SCOMx->SCOM_Operating.rxCallbackFun   = SCOMx_InitStruct->rxCallbackFun;
		SCOMx->SCOM_Operating.txCallbackFun   = SCOMx_InitStruct->txCallbackFun;
